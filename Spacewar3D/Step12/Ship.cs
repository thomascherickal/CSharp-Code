...
public class Ship {
	...
	private ParticleEffects vaporTrail;
	...
	public void UpdatePosition(float elapsedTime) {
		...
		if (state == ShipState.Dying) {
			vaporTrail.UpdateThrustEffect(elapsedTime,0,Color.White,Color.Black,
				this.Position.CombinedLocation);
		...
		int numTrailParticles = 4;
		vaporTrail.UpdateThrustEffect(elapsedTime,numTrailParticles,Color.White,Color.Black,
			this.Position.CombinedLocation);
