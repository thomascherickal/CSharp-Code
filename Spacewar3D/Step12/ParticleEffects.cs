...
public class ParticleEffects {
	...
	/// <summary>
	/// Updates the explosion effects
	/// </summary>
	public void UpdateExplosion(int NumParticlesToEmit, Vector3 vPosition) {
		// Emit new particles
		...
		while(particles < particlesLimit && particles < particlesEmit) {
			...
			// Emit new particle
			float Rand1 = ((float)rand.Next(int.MaxValue)/(float)int.MaxValue) * (float)Math.PI * 2.0f;
			float Rand2 = ((float)rand.Next(int.MaxValue)/(float)int.MaxValue) * (float)Math.PI * 2.0f;

			particle.initialPosition = vPosition;

			particle.initialVelocity.X  = (float)Math.Cos(Rand1) * (float)Math.Sin(Rand2) * 100f;
			particle.initialVelocity.Z  = (float)Math.Sin(Rand1) * (float)Math.Sin(Rand2) * 100f;
			particle.initialVelocity.Y  = (float)Math.Cos(Rand2) * 100f;
